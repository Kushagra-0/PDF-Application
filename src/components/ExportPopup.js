        ...
        <Dialog as="div" className="relative z-50" onClose={props.setOpen}>
          ...
          <div className="fixed inset-0 z-10 overflow-y-auto">
            <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0 w-full">
              ...
              >
                <Dialog.Panel className={"my-6 relative transform overflow-hidden rounded-lg  px-4 pt-5 pb-4 text-left shadow-xl transition-all bg-white"}>
                  <Backdrop
                            ...
                            <div className="w-full py-4">
                              <div ref={contextValues.exportPage} id="toExport" style={{ opacity: currPage <= numPages ? '1' : '0' }}>
                                <Document file={contextValues.selectedFile} onLoadSuccess={onDocumentLoadSuccess} className="flex justify-center">
                                  ...
                                  <Page pageNumber={currPage} id="docPage" className={`px-4 py-2 ${!isExporting && 'shadow-lg border'} border-[rgba(36,36,36,0)]"}`} width={595} height={842} />
                              ...
                              <div className='fixed top-1 flex items-center justify-center w-full gap-3 mt-3 opacity-70' style={{ opacity: currPage <= numPages ? '1' : '0' }}>
                                {currPage > 1 && <button onClick={() => changePage(-1)} className='px-2 py-1 text-sm bg-black rounded-md text-white'>{'<'}</button>}
                                <div className='px-2 py-1 text-sm bg-black rounded-md text-white'>Page {currPage} of {numPages}</div>
                                {currPage < numPages && <button onClick={() => changePage(1)} className='px-2 py-1 text-sm bg-black rounded-md text-white'>{'>'}</button>}
                              </div>
                            </div>
                            : null}
                        </div>
                  ...
                  <div className="mt-5 sm:mt-6">
                    <button
                      type="button"
                      className="inline-flex w-full justify-center rounded-md border border-transparent bg-black px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-grey-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:text-sm"
                      onClick={() => onExport()}
                    >
